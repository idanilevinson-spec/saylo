...
**Softened fields, full-strength accents.** The first build of this pass ran the hero and final-CTA sections at full-saturation `--primary`/`--accent` across the whole field — direct user feedback: too glaring across a whole section, even though the same intensity is exactly right on a small button or badge. Added `--primary-soft` / `--accent-soft` (`color-mix(in srgb, var(--primary) 45%, var(--background))`, same for accent), used only for large full-bleed areas: the hero section background, its studio split panel, the final-CTA section, the ticker. Text sitting on these fields moved from the ink tokens (tuned for full-saturation contrast) to `--foreground`, since the softened field sits much closer to the page's own background luminance. Small elements — the primary CTA button, the LIVE dot, the plates' color-coded edge bars, badges — stay at full `--primary`/`--accent` strength on purpose: a calm field with one or two vivid focal points reads better than either uniformly loud or uniformly muted. Referencing `--primary`/`--accent`/`--background` directly (not hardcoded per theme) means the soft tokens automatically track whichever theme is active without their own redeclaration.
