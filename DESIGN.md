...
**Shape consistency, extended app-wide.** `rounded-2xl` (the pre-"On Air" card radius) was still sitting in 45 files across the whole codebase — dashboard, every exercise page, admin, shared components like `ContentCard` (which alone drives the card grid on vocabulary/grammar/reading/listening/writing/idioms/learn). Swapped to `rounded-lg` everywhere, matching the radius the landing page and auth funnel already committed to. Small icon-badge circles (`w-8`-`w-12`) keep their own smaller `rounded-xl`/`rounded-lg` intentionally — a smaller radius on a smaller element isn't the inconsistency this was fixing, a second *card* radius system running in parallel was. The dashboard's greeting card also picked up the same primary-colored top edge every other plate on the site now carries (hero, correction demo, pricing teaser, the auth forms) — one recurring detail reused, not invented per surface.
