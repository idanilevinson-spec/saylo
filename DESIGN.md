...
**Softened fields, full-strength accents.** An early build of this pass ran the hero and final-CTA sections at full-saturation `--primary` across the whole field — direct user feedback: too glaring across a whole section, even though the same intensity is exactly right on a small button or badge. `--primary-soft` (`color-mix(in srgb, var(--primary) 45%, var(--background))`) is used only for large full-bleed areas: the hero section background, the final-CTA section, the ticker. Text sitting on these fields uses `--foreground` (or the opposite pole, `--background`, on the fully-saturated bookend fields — see below), not the ink tokens tuned for full-saturation contrast. Small elements — the primary CTA button, the LIVE dot, the plates' edge bars, badges — stay at full `--primary` strength on purpose: a calm field with one vivid focal point reads better than either uniformly loud or uniformly muted. `--primary-soft` references `--primary`/`--background` directly, so it tracks whichever theme is active automatically, no redeclaration per theme needed.

**A blend ratio that reads safely in one theme can fail in the other.** `--primary-soft` at 45% measured 5.58:1 for foreground-toned text in dark mode but only 3.73:1 in light mode for the same class — the two themes' underlying foreground/background luminance relationship isn't symmetric, so a "medium" blend lands in a contrast no-man's-land in at least one theme. Fixed by keeping text-bearing full-bleed sections (the ticker, the final CTA) at the fully-saturated `--primary` field with `--foreground`/`--background`-pole text, reserving the soft blend for backgrounds nothing sits directly on top of as text. Verified via canvas-composited pixel sampling (not raw `getComputedStyle` colors, which return unparseable `color(srgb ...)` / `oklab(...)` strings for `color-mix` results and opacity-modified colors) in both themes: hero headline vs. plate 14.6:1 (dark); the ticker 7.4-7.97:1; the pricing badge 5.42-6.79:1. All comfortably above WCAG AA.

Color strategy on the landing page: **Committed** — the hero and final CTA are solid or near-solid primary fields, not accents scattered on a neutral ground. Operate surfaces (dashboard, exercises, admin) stay **Restrained** as before, with both primary and accent available — this redesign was scoped to the marketing/landing page only.
