...
// useSearchParams() (for the ?topic= deep link) needs a Suspense boundary
// on this route since it has no dynamic segment of its own — same reason
...
function SpeakingTestPageInner() {
  ...
  const [steps, setSteps] = useState<SpeakingTestStep[]>([]);
  ...
  useEffect(() => {
    if (initialTopicSlug && topics.length > 0) {
      ...
      if (topic) generate(topic.id, topic.name_en);
    }
  ...
  async function generate(topicId?: string, topicNameEn?: string) {
    if (!profile) return;
    setPhase("generating");

    let query = supabase.from("vocabulary_items").select("id, headword, translation_he").eq("status", "published");
    query = topicId ? query.eq("topic_id", topicId) : query.eq("cefr_level", (await currentLevel(profile.id)) ?? "A1");
    ...
    // Open questions are generated fresh per attempt (not stored) — the
    // same variety principle used elsewhere in this app, since a fixed
    // bank of speaking prompts would get memorized fast. A failure here
    // isn't fatal: buildSpeakingTest just gets fewer/no open steps.
    const questionsRes = await fetch("/api/ai/speaking-test-questions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ topicNameEn }),
    });
    const questionsData = questionsRes.ok ? await questionsRes.json() : { questions: [] };

    setSteps(buildSpeakingTest(pool, questionsData.questions ?? []));
    setPhase("ready");
  }
  ...
  if (phase === "ready") {
    return <SpeakingTest steps={steps} />;
  }

  if (phase === "generating") {
    return <div className="max-w-xl mx-auto px-4 py-24 text-center text-muted">בונים לכם מבחן דיבור...</div>;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <button
        onClick={() => generate()}
        className="mt-6 w-full flex items-center gap-4 bg-primary/10 border border-primary/25 rounded-lg p-5 hover:border-primary/45 hover:shadow-md transition-all text-right focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
      >
      ...
      <div className="mt-6 space-y-3">
        {topics.map((topic, i) => (
          ...
          >
            ...
            <Mic size={18} className="text-muted shrink-0" />
          </motion.button>
        ))}
      </div>
    </div>
