...
describe("isPaidActive", () => {
  ...
  it("is not active during an unexpired trial — the AI Teacher gate excludes the trial", () => {
    const sub = { status: "trialing" as const, trial_ends_at: "2026-06-16T12:00:00Z", current_period_end: null };
    expect(isPaidActive(sub, now)).toBe(false);
  });
  ...
  it("is not active for expired or past_due status", () => {
    expect(isPaidActive({ status: "expired", trial_ends_at: null, current_period_end: null }, now)).toBe(false);
    expect(isPaidActive({ status: "past_due", trial_ends_at: null, current_period_end: null }, now)).toBe(false);
  });
});
