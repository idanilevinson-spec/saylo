...
For each question, also write a short model answer (2-4 spoken-style sentences, same CEFR level) — since these questions are open-ended with no single correct answer, this model answer is shown to a learner who skips the question, as an example of what a good answer sounds like, not the "right" one.

Respond with ONLY valid JSON, no markdown code fences, in exactly this shape:
{"questions": [{"questionEn": "<question 1>", "modelAnswerEn": "<model answer 1>"}, ...]}`;
}
